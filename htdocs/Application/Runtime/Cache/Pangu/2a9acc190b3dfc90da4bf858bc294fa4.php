          ...
          <th width="120">操作</th>
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td><?php echo ($vo["tech"]); ?></td>
          <td><?php echo ($vo["url"]); ?></td>
					<td class="f-14 td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="admin_edit('编辑','<?php echo U('Index/edit_copyright');?>?id=<?php echo ($vo["id"]); ?>','','')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
					</td>
...
$(function(){
  ...
  $(".w_del").click(function(){
    ...
    $.post("<?php echo U('Index/do_del');?>",{id:id,table:"goods"},function(data){
      if (data==1) {
      alert("删除成功")     
