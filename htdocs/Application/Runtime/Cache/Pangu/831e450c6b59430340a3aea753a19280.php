...
<div class="pd-20">
  ...
  <a class="btn btn-primary radius" onclick="admin_add('添加','<?php echo U('Pangu/Index/edit');?>?table=case_type&type=<?php echo ($type); ?>','800','800')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
   </span> 
  </div>
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td class="f-14 td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="admin_edit('编辑','<?php echo U('Index/edit_case_type');?>?id=<?php echo ($vo["id"]); ?>','','')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
					</td>
...
$(function(){
  $(".del_all").click(function(){
      ...
      $(".hidden-xs").each(function(){
          ...
          $.get("<?php echo U('Index/do_del_all');?>?table=case_type&ids="+ids,function(data,s){
  ...
  $(".w_del").click(function(){
    ...
    $.post("<?php echo U('Index/do_del');?>",{id:id,table:"case_type"},function(data){
      if (data==1) {
      alert("删除成功")     
