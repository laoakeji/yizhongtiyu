...
<div class="pd-20">
	...
	<a class="btn btn-primary radius" onclick="admin_add('添加','<?php echo U('Pangu/Index/edit');?>?table=img&type=<?php echo ($type); ?>','800','800')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
	 </span> 
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td><img src="<?php echo ($vo["img_url"]); ?>" style="width:120px;height: 60px"></td>
					<td><?php echo ($vo["title"]); ?></td>
					<td><?php echo ($vo["link"]); ?></td>
					<td>
						<?php if(($vo["type"]) == "1"): ?>轮播盘<?php endif; ?>
						<?php if(($vo["type"]) == "2"): ?>关于亿众<?php endif; ?>
						<?php if(($vo["type"]) == "3"): ?>产品中心<?php endif; ?>
						<?php if(($vo["type"]) == "4"): ?>行业动态<?php endif; ?>
						<?php if(($vo["type"]) == "5"): ?>经典案例<?php endif; ?>
						<?php if(($vo["type"]) == "6"): ?>联系我们<?php endif; ?>
					</td>
					<td><?php echo (date("Y-m-d H:i:s",$vo["c_time"])); ?></td>
					<td><?php echo ($vo["sort"]); ?></td>
					<td class="f-14 td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="admin_edit('编辑','<?php echo U('Index/edit');?>?table=img&type=<?php echo ($type); ?>&id=<?php echo ($vo["id"]); ?>','800','500')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
