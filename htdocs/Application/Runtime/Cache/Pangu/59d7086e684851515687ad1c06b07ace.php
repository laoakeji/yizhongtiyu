  ...
  <form id="form2" method="post" class="form form-horizontal" id="form-article-add">
   ...
   <input type="hidden" name="type" value="<?php echo ($type); ?>"></input>
    ...
    <div class="row cl">
      ...
      <p class="" id="file_upload1"><span>上传</span></p><label>预览：</label><img id="img1" style="hight:64px;width:64px" src="<?php echo (is_img($list["src"])); ?>"><input type="hidden"  id="img_url1" name="src" value="<?php echo (is_img($list["src"])); ?>">
    </div>

    <div class="row cl">
      <label class="form-label col-2">图片1：</label>
      <div class="formControls col-2"> 
      <p class="" id="file_upload2"><span>上传</span></p><label>预览：</label><img id="img2" style="hight:64px;width:64px" src="<?php echo (is_img($list["img1"])); ?>"><input type="hidden"  id="img_url2" name="img1" value="<?php echo (is_img($list["img1"])); ?>">
    </div>

    <div class="row cl">
      <label class="form-label col-2">图片2：</label>
      <div class="formControls col-2"> 
      <p class="" id="file_upload3"><span>上传</span></p><label>预览：</label><img id="img3" style="hight:64px;width:64px" src="<?php echo (is_img($list["img2"])); ?>"><input type="hidden"  id="img_url3" name="img2" value="<?php echo (is_img($list["img2"])); ?>">
    </div>
  ...
  <script type="text/javascript">
      $(function(){
        $('#sub_form2').click(function(){
          ...
          var id="<?php echo ($id); ?>";
          $.post("<?php echo U('Index/do_edit');?>?table=pro_detail&id="+id,data,function(data){
            if (data>=1) {
              //layer.msg("修改成功");
              alert("修改成功");
