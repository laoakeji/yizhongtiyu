...
<div class="pd-20">
  ...
  <a class="btn btn-primary radius" onclick="admin_add('添加','<?php echo U('Pangu/Index/edit');?>?table=intro_year&type=<?php echo ($type); ?>','600','350')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
   </span> 
  </div>
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
          ...
          <th width="120">操作</th>
			...
			<tbody>
				<?php if(is_array($yearlist)): $i = 0; $__LIST__ = $yearlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td class="f-14 td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="admin_edit('编辑','<?php echo U('Index/edit_intro_year');?>?id=<?php echo ($vo["id"]); ?>','500','300')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
...
<div class="cl pd-5 bg-1 bk-gray mt-20"> 
  <span class="l"><a href="javascript:;" id="" class="btn btn-danger radius del_all1"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
  <a class="btn btn-primary radius" onclick="admin_add('添加','<?php echo U('Pangu/Index/edit_intro_story');?>','600','350')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
   </span> 
  </div>
<div class="mt-20">
    <table class="table table-border table-bordered table-bg table-hover table-sort">
      ...
      <tbody>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
          <td class="hidden-xs" aid="<?php echo ($vo["id"]); ?>"><input type="checkbox" value="<?php echo ($vo["id"]); ?>" aid="<?php echo ($vo["id"]); ?>" name=""></td>
          <td><?php echo ($vo["id"]); ?></td>
          <td><?php echo ($vo["yearid"]); ?></td>
          <td><?php echo (date("Y-m-d",$vo["time"])); ?></td>
          <td><?php echo ($vo["content"]); ?></td>
          <td class="f-14 td-manage">
            <a style="text-decoration:none" class="ml-5" onClick="admin_edit('编辑','<?php echo U('Index/edit_intro_story');?>?id=<?php echo ($vo["id"]); ?>','500','500')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
            <a style="text-decoration:none" class="ml-5 w_del1" aid="<?php echo ($vo["id"]); ?>" href="javascript:void(0)"><i class="Hui-iconfont">&#xe6e2;</i></a>
          </td>
        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
      </tbody>
    </table>
...
$(function(){
  $(".del_all").click(function(){
      ...
      $(".hidden-xs").each(function(){
          ...
          $.get("<?php echo U('Index/do_del_all');?>?table=intro_year&ids="+ids,function(data,s){
  ...
  $(".del_all1").click(function(){
      ...
      $(".hidden-xs").each(function(){
          ...
          $.get("<?php echo U('Index/do_del_all');?>?table=intro_story&ids="+ids,function(data,s){
  ...
  $(".w_del").click(function(){
    ...
    $.post("<?php echo U('Index/do_del');?>",{id:id,table:"intro_story"},function(data){
      if (data==1) {
      alert("删除成功")     
